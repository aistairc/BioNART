...
json_dir = "/scratch/aae15163zd/data/bio/prepared_data/healthcaremagic"
#json_dir = "/scratch/aae15163zd/data/bio/prepared_data/icliniq"
...
train_data = train_data.map(
    ...
    batched=True,
    batch_size=1024,
    remove_columns=["src", "tgt", "id"],
    #remove_columns=["CHQ", "Summary", "File"],
...
def randomize_model(model):
    ...
    for module_ in model.named_modules():
        if isinstance(module_[1],(torch.nn.Linear, torch.nn.Embedding)):
            module_[1].weight.data.normal_(mean=0.0, std=initializer_range)
...
#run_name = "icliniq-1e-4-drop0.5-from-random"
run_name = "hcm-5e-5-drop0.5-from-pre"
